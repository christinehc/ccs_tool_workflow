...
# load models
model = {k: darkchem.utils.load_model(MODEL_PATH[k]) for k in MODEL_PATH.keys()}
...
for adduct in snakemake.config["adducts"]:
    ...
    x_latent = model[adduct].encoder.predict(x)

    # generate property predictions
    y_pred = model[adduct].predictor.predict(x_latent)
